...
- **Phase 4 (mostly done)** — P4-1, P4-2, P4-4, P4-5, P4-6, P4-7, P4-8, P4-11, P4-13, P4-14, P4-15, P4-16, P4-17, P4-18, P4-19, P4-20, P4-21, P4-22, P4-23, P4-25, P4-27, P4-28. Plus the **UI batch verified against the running app**: P4-29 (search-debounce untrack), P4-30 (WS reconnect resync), P4-31 (reopen status + mutation catches), P4-32 (omni error/space/version/stale), P4-33 (WS receive loop → tasks; verified live), P4-34 (Day Summary dedupe + CardDetail refetch guard), P4-35 (coherence guards), P4-36 (shortcuts/ws-close/WsMessage). P4-24's core (atomic `executing`) landed via P3-8.
...
- **Phase 7 (started)** — P7-1 *part 1* (shared retrieval primitives: `laya/retrieval.py` with STOPWORDS + extract_keywords + reciprocal_rank_fusion; fixed the RRF doc-id drift and the search_cards FTS-OR/LIKE-AND divergence via `build_fts_match(match_all=...)`). P7-2 (spec-driven persona worker: 5 near-verbatim drafting workers → `workers/persona.py` run_persona_worker + PERSONA_SPECS; hoisted the triplicated `_summarize_findings`; ops/finance drift now explicit config). P7-3 (concrete `BaseCodingAgent` in agents/base.py holds the shared lifecycle — `_make_event`, control methods, exit-code epilogue — for the 4 CLI adapters; Claude's MCP cleanup is now an `_on_process_exit` hook). P7-9 (removed the dead 157-line `oauth._setup_n8n_workflows` + its third copy of the credential-injection matcher; shared `_PLATFORM_HTTP_CRED_TYPES` dict kept). P7-10 (Rust `process_util.rs`: `home_dir`/`laya_home` ×3 and the LD_LIBRARY_PATH scrub ×2 collapsed to one copy each; the shutdown SIGTERM loops left for a live-verified pass). P7-4 (route status writes through `transition_card_status`: `allow_restore` flag; reopen_card's 4 blocks → 1 SSOT call; card_tools + pipeline-fail routed; agent-streaming writes left for P7-6). P7-5 (merged `llm_call`/`llm_call_streaming` prep into `_prepare_call_kwargs` — streaming now gets the max_tokens clamp it was missing).

**⏳ Remaining (not yet done):**
- **Phase 4** — P4-9 (`/cards/grouped` unbounded — **bug confirmed live: 55.9 MB / 6694 groups / 20,889 cards with the date filter dropped**; the fix is a coordinated backend cap + pagination + detail lazy-fetch and a product call on what all-days shows — deferred as needing browser-driven verification, not a blind response-shape change), P4-10 (route feed search through FTS — changes search results), P4-12 (txn-isolation guards — delicate, touches shared-connection semantics); P4-24 composer `/egress/execute` dedup (core covered by P3-8); P4-26 (WS `send_input` no-op — the workspace agent-answer flow, wants agent-driven verification).
...
- **Phase 7** — P7-1 *part 2* (fold the FTS-else-LIKE search *bodies* into `retrieval.py` — changes search results, wants live verification), P7-6 (split the 2,800-line `cards_api.py` + merge the two agent-streaming functions), P7-7 (extract the 2,617-line `feed/+page.svelte`), P7-8 (learn/context_learn shared driver — tied to the deferred P4-8 consolidation half).

Remaining: (a) three riskier/UI-coupled engine items (P4-9/10/12) that want browser-driven verification of the feed, (b) the big structural splits (P7-6/7), (c) P7-8 and the P6 prompt program.
...
| ID | Finding (ref) | Files | Fix | Effort |
|---|---|---|---|---|
| **P4-9** | `/cards/grouped` unbounded — no LIMIT, ships heavy JSON per card, UI drops `date` filter in bookmarked/related/all-days modes → multi-MB, blocks shared DB conn (§2, §4) | `api/cards_api.py`, feed caller | Cap distinct groups server-side (paginate/limit); stop shipping `intelligence`/`staged_output`/`suggested_actions` in the list payload (lazy-load on detail). | M |
| **P4-10** | Feed search bypasses FTS5 — 16 `LIKE`s + correlated tag subquery per row per term (§2, §4) | `api/cards_api.py:271-296` | Route feed search through existing `build_fts_match` (`cards_fts`). | M |
...
| **P4-12** | Single shared aiosqlite conn → zero txn isolation; no `rollback()` anywhere; false "ONE transaction" comment (§2) | `db/*`, `_persist_card` | Guard the few multi-statement invariants (card cascade, space delete, merge) with a lock + explicit `BEGIN/COMMIT/ROLLBACK`. Do **not** attempt per-request isolation on the shared conn — scope to invariants. | M |
...
| **P7-8** | `learn.py` / `context_learn.py` structurally near-identical; tuning defaults duplicated (`DEFAULT_SETTINGS["tuning"]` vs inline `get_tuning` literals) (§5.9) | Share a driver once consolidation is ported to the classification side (P4-8). Centralize tuning defaults. | M |
