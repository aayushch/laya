...
- **Phase 4** — P4-9 (`/cards/grouped` unbounded — **bug confirmed live: 55.9 MB / 6694 groups / 20,889 cards with the date filter dropped**; the fix is a coordinated backend cap + pagination + detail lazy-fetch and a product call on what all-days shows — deferred as needing browser-driven verification, not a blind response-shape change), P4-24 composer `/egress/execute` dedup (core covered by P3-8); P4-26 (WS `send_input` no-op — the workspace agent-answer flow, wants agent-driven verification).
...
Remaining: (a) P4-9 (`/cards/grouped` cap + pagination + payload slim — product call made: "cap + load more"; needs coordinated backend + UI + browser verification), (b) the big structural splits (P7-6/7), (c) the deferred P7-8 driver/consolidation half and the P6 prompt program.
