...
| **P7-7** ◐ | `feed/+page.svelte` (2,701 lines) (§5.5) | ◐ *Shared-map dedup done*: `timeAgo` (6 divergent copies → one `datetime.timeAgo(opts)` with weeks/nullLabel) and `PRIORITY_LABELS` (6 byte-identical copies → `cardVisuals`). **Deferred** (each needs a call or carries UI risk): `priorityColors` unification (**genuinely divergent** — Family A amber/rose vs B orange/coral, changes pixels → needs a canonical pick); FLIP/layout engine (timing-sensitive, `_flipSettled` contract); the `card_updated` WS reducer (highest bug-density — extract behind unit tests); component carves (SummaryModal/FilterPopover/RecentDrawer). `statusLabel`/`statusColors` intentionally left as separate card-vs-group exports (vocabularies genuinely differ — not a mechanical merge). | L |
