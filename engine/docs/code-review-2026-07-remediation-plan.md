...
- **Phase 7** — P7-7 *deeper pieces* (the maps are done — `timeAgo`/`PRIORITY_LABELS`/`PRIORITY_COLORS`; remaining: extract the FLIP/layout engine, the `card_updated` WS reducer behind unit tests, and the component carves SummaryModal/FilterPopover/RecentDrawer — all MED risk, want browser-driven feed verification). P7-8 *remaining* (the full extraction-loop driver merge over `run_learn_extraction`/`run_context_learn_extraction` + porting context-rules' LLM consolidation pass to the classification side — the deferred P4-8 half).

Everything else in the review is done: Phases 1–3 + 5 complete, Phase 6 accounting complete, Phase 4 complete (P4-24 mitigated), Phase 7 = P7-1..P7-6 + P7-9/P7-10 done, P7-7 maps done, P7-8 safe parts done. Intentionally NOT unifying `personaColors` decorated variants or `statusLabel`/`statusColors` (card-vs-group vocabularies genuinely differ).
...
| ID | Finding (ref) | Files | Fix | Effort |
|---|---|---|---|---|
| **P4-9** ✓ | `/cards/grouped` unbounded — no LIMIT, ships heavy JSON per card, UI drops `date` filter in bookmarked/related/all-days modes → multi-MB, blocks shared DB conn (§2, §4) | `api/cards_api.py`, feed caller | ✓ *Part a (payload slim)*: list drops `staged_output`/`suggested_actions` (`_row_to_card(slim=True)`), lazy-loaded on detail via `GET /cards/{id}`; `intelligence` kept for client-search parity; 3 grouped-card UI consumers fixed. ✓ *Part b (cap + paginate)*: server caps to `limit`=200 groups + `offset`/`has_more`; feed `loadMoreGroups()` appends (dedup by entity_id) + "Load more" button. Live: 3.30 MB → 22 KB first page. | M |
...
| **P4-24** ✓~ | Execution idempotency is non-atomic check-then-act; `/egress/execute` (composer) has no dedup (§2) | egress execute paths | ✓ Atomic `executing` transition landed via P3-8. Composer double-click is already prevented at the FE: `ComposeModal`, `QuickActions`, `InlineEditor` all disable their action button on a `sending`/loading flag during the request. Residual — server-side idempotency for a network-retry/second-client — is **low practical exposure** (there's no client auto-retry) and left as a deferred belt-and-suspenders. | M |
...
| ID | Finding (ref) | Files | Fix | Effort |
|---|---|---|---|---|
| **P4-26** ✓ | WS agent input is a silent no-op (stdin `DEVNULL`); UI records input as delivered (§2) | `api/ws_router.py`, `api/workspace_api.py` | ✓ Extracted `workspace_api.resume_session_with_answer()` and routed both the HTTP `/answer` endpoint and the three WS handlers (approve/deny/user_input) through it — answers now `--resume`-spawn instead of writing to DEVNULL stdin. `test_ws_router` updated + `test_workspace_api` green. Live agent-Q&A verification pending. | S |
...
| ID | Finding (ref) | Fix | Effort |
|---|---|---|---|
| **P6-8** ✓ | Chat tool definitions ~8.4K tok, resent every turn + every tool-loop iteration (cap 20) — single largest cost; exceeds an 8K local window before the question (§3) | ✓ *Gating*: measured 8,404 tok total → `select_chat_tools()` always ships read+write (2,806 tok), gates settings/rules/egress behind an inclusion-biased keyword intent check over the current + recent user turns. Ordinary turn now **8,404 → 2,806 tok (67% smaller)**; intent turns pull in only the one relevant group. ✓ *Vocabulary move (part 2)*: `create_filter_rule`/`create_processing_rule` no longer restate the field/operator lists inline — `get_rule_options` now serves `fields`/`operators` keyed by rule type (filter vs processing) from canonical constants; the enforcing operator enum stays in-schema. rules-write group 1,995 → 1,862 tok (-133/rules-intent turn); full catalog → 8,297. Golden-contract tests in `test_tool_gating.py`. | L |
| **P6-9** ◐ | Stager system prompt (~3.3K/event) ~45% conditionally irrelevant (email-phishing on Jira events, PR-lifecycle on emails, ~900 tok of role-context explanation) (§3) | ◐ *Stager platform-gated*: extracted the email links/phishing, issue_* update, and PR-lifecycle blocks (896 tok, 29% of the prompt) into `{placeholders}` assembled per platform family by `build_stager_system_prompt(platform)` — round-trip verified byte-identical, golden tests pin universal directives present + per-family gating. Per-event saving 231 tok (email) → 660 tok (chat/calendar). ✓ *Router platform-gated* too: Jira issue-lifecycle + Bitbucket/GitHub PR-lifecycle blocks → `build_router_system_prompt(platforms)` (batch router unions its events' platforms); gmail/slack save 583 tok, jira/github 258–324. Both round-trip verified + golden tests. **Combined router+stager saving ≈ 660–1,243 tok on every pipeline event.** *Remaining*: the ~900-tok stager role-explanation trim (content-condensing — deferred). | M |
| **P6-10** ✓ | Router emits a 3–5-step `research_plan` even when `requires_research=false` (§3) | ✓ Prompt now instructs an empty `research_plan []` for simple notifications (the output saving), and `run_router` enforces `research_plan=[]` when `requires_research=false` after parse (guards small models that ignore it + keeps stored output clean). Schema field documented. `test_router` pins both cases. | S |
...
| **P6-13** ✓ | Omni resynthesis most truncation-prone (full snapshot + up to 150 cards ≈ 8–15K tok; failure grows the next attempt) (§3, §4) | ✓ New cards fold across sequential calls of `_RESYNTH_CHUNK_SIZE` (40) each — oldest chunk first so the newest lands in the final call; snapshot-relative prune hints apply to the first fold only; pins carry through. Small batches still run as one call (byte-identical). Any chunk parse-failure discards the run without advancing the since-watermark, so all cards retry (no partial-store card loss). Tests in `test_omni_resynthesis.py` pin chunk count/ordering/feed-forward + failure-discard. | M |
...
| **P7-6** ✓ | `cards_api.py` (2,957 lines) splits along ~8 seams; 30-column card SELECT duplicated 4× and drifted (trace_api omits `read_at`/`context_id`) (§5.4) | ✓ *Streams merged* into `_run_agent_session_stream`. ✓ *File split*: **2,957 → 131 lines**, decomposed into `cards_common` (shared `_row_to_card`) + `cards_feed`/`_lifecycle`/`_readstate`/`_payload`/`_agent`/`_groups`. `cards_api` is now an aggregator that `include_router`s them IN PLACE (route order byte-identical — verified: same 206 route entries, `/cards/grouped` still before `/cards/{card_id}`) and re-exports the 5 externally-imported symbols (zero change to the 8 importers). Unused imports trimmed. ✓ *Canonical card-SELECT*: the 30-column projection (copied into 5 SELECTs that had drifted — trace_api dropped read_at/context_id, get_card dropped context_id/content_metadata, trace dropped read_at/group_active_at/context_id/content_metadata) is now one `cards_common.CARD_SELECT_COLUMNS`; every path feeds `_row_to_card` the full shape (verified: 35 cols resolve against the migrated schema, the drifted fields now populate). | L |
| **P7-7** ✓ | `feed/+page.svelte` (2,701 lines) (§5.5) | ✓ *Shared-map dedup*: `timeAgo`/`PRIORITY_LABELS`/`PRIORITY_COLORS` (Family A amber/rose canonical). ✓ *`card_updated` WS reducer* → pure `cardUpdateReducer.ts` (returns `{groups, selectedCard, effects}`) behind **26 vitest tests** (stood up vitest for the repo). ✓ *FLIP engine* → `utils/flip.ts` (`capturePositions`+`playFlip`); `animateFlip`/`flipColumns`/recent-drawer all delegate, `_flipSettled` preserved. ✓ *Component carves*: `SummaryModal`, `FilterPopover` (owns `toggleFilter`, dropped `filterMenuEl`), `RecentDrawer` (owns its reorder FLIP). **feed/+page.svelte 2,701 → 2,298 lines.** `statusLabel`/`statusColors` intentionally left as separate card-vs-group exports (vocabularies genuinely differ — not a mechanical merge). | L |
| **P7-8** ✓ | `learn.py` / `context_learn.py` structurally near-identical; tuning defaults duplicated (`DEFAULT_SETTINGS["tuning"]` vs inline `get_tuning` literals) (§5.9) | ✓ Tuning defaults centralized; ✓ `get_spaces_with_unprocessed` + now the shared `fetch_unprocessed_corrections` / `mark_corrections_processed` query patterns extracted to `learn_common.py` (both drivers use them). A *full* single-driver merge was assessed and rejected — the two differ in field-handling / parse-fallback / error-handling / consolidation, so a unified driver would need ~6 callbacks, harder to follow than two focused functions. ✓ **P4-8 consolidation port**: `maybe_consolidate_classification_rules` mirrors the context side (new `classification_rule_consolidator` prompt, field-aware merge within priority/persona, transactional replace + guardrails), called at the end of `run_learn_extraction`; new `classification_rules_consolidation_threshold` (40). The `max_injection` cap was already in `query_classification_rules`. Covered by `test_learn_consolidation.py` (consolidation guardrails + field preservation + scope + end-to-end driver). | M |
