...
Remaining: (a) the big structural splits (P7-6/7), (b) the deferred P7-8 driver/consolidation half, (c) the P6 prompt-reduction program (needs a golden-output harness first), and the small P4-24 (composer dedup; core already covered by P3-8).
...
| **P7-6** ◐ | `cards_api.py` (2,957 lines) splits along ~8 seams; 30-column card SELECT duplicated 4× and drifted (trace_api omits `read_at`/`context_id`) (§5.4) | ◐ *Streams merged*: `_stream_agent_to_card` + `_stream_entity_agent`'s ~110 near-identical lines → shared `_run_agent_session_stream(session_id, agent, broadcast_card_id, *, log_prefix)`; each caller keeps only its unique preamble (spawn vs caller-owned). Remaining: split into feed/lifecycle/read-state/payload/agent/context-group modules via an **aggregator** `cards_api` shell that re-exports the 5 external symbols (zero blast radius to the 8 importers); single canonical card-SELECT. | L |
