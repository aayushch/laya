...
@pytest.mark.asyncio
class TestExecutor:
    async def test_execute_success(self, db_m8):
        """Successful execution updates card to completed and stores action_log."""
        await _insert_test_card(db_m8, "card_exec", "evt_exec", status="pending")
    ...
    async def test_execute_card_not_found(self, db_m8):
    ...
    async def test_execute_wrong_status(self, db_m8):
        """Raises ValueError when card status is not pending or approved."""
        await _insert_test_card(db_m8, "card_done", "evt_done", status="completed")
    ...
    async def test_execute_action_not_found(self, db_m8):
        """Raises ValueError when action_id does not match any suggested action."""
        await _insert_test_card(db_m8, "card_noact", "evt_noact", status="pending")
    ...
    async def test_execute_modifications_merged(self, db_m8):
        """User modifications are merged into the action payload."""
        await _insert_test_card(db_m8, "card_mod", "evt_mod", status="pending")
    ...
    async def test_execute_broadcasts_twice(self, db_m8):
        """Broadcasts card_updated twice: once for 'executing', once for final status."""
        await _insert_test_card(db_m8, "card_bc", "evt_bc", status="pending")
    ...
    async def test_execute_from_approved_status(self, db_m8):
        """Execution works from 'approved' status too."""
        await _insert_test_card(db_m8, "card_app", "evt_app", status="approved")
