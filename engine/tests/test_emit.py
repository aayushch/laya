...
class TestEmit:
    async def test_creates_card_in_db(
        self, db_m4, sample_event, sample_router_output_engineer,
    ):
        """run_emit inserts a card into action_cards table."""
        # Insert parent event row first (FK constraint)
        await db_m4.execute(
            ...
            (sample_event.event_id, "2026-02-22T14:30:00Z", "jira", "issue_assigned",
    ...
    async def test_broadcasts_card_created(
        ...
        assert broadcast_msg["type"] == "card_created"
    ...
    async def test_no_workspace_without_session(
        self, db_m4, sample_event, sample_router_output_comms,
