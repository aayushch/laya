...
## Phase 13: Inference Backends, Hybrid Retrieval & Automation (Jun 2026)

| # | Decision | Rationale |
|---|---|---|
| 75 | **CLI agents as inference backends** | A pipeline stage can run on an installed CLI agent (Claude Code / Codex / Gemini / Pi) via model ids of the form `agent/<id>/<model>`, so users run on the agent's own subscription/quota instead of an API key. Claude Code is "native" (JSON schema via `--json-schema`); others are "best-effort" (schema-as-text + validate/retry). Agents run headless in an ephemeral cwd with built-in tools denied. |
| 76 | **Window-based agent usage budget** | Agents bill against time-windowed quotas (not `$`), so agent budgeting is separate from the monthly cost budget: when an agent's rolling window passes a pause threshold (default 85%), ingestion auto-pauses and auto-resumes at the window reset. Native rate-limit signals (Claude Code's `rate_limit_event`) feed the same logic. |
| 77 | **Hybrid retrieval (vector + BM25)** | Chat and trace retrieval combine ChromaDB vector search with SQLite FTS5/BM25 lexical search, merged via Reciprocal Rank Fusion, so exact-keyword matches surface alongside semantic ones. FTS tables (`cards_fts`, `events_fts`) are built at startup and kept in sync by SQL triggers, with a `LIKE` fallback when FTS5 is unavailable. |
| 78 | **Contextual BM25 via thread_context** | EMIT persists a short thread blurb (rolling group-summary headline or recent card headers) to `action_cards.thread_context`, indexed by FTS5, so terse follow-up cards ("Approved.") stay findable by their thread's keywords. Forward-only; NULL for first/pre-existing cards. |
| 79 | **Context rules, learned + consolidated** | Natural-language context rules (distinct from classification rules) guide context-association grouping. They are learned from link/unlink corrections and, once a scope's learned rules exceed a threshold, an LLM consolidator merges redundant ones while preserving manual rules. Managed via `/context-rules`. |
| 80 | **Chat-driven rule management** | Filter, classification, and processing rules can be created/updated/deleted directly from chat (`rules_tools.py`); changes are audited and broadcast over WebSocket (`rules_changed`). |
| 81 | **Egress platform classes (supersedes #70)** | Each platform now owns its full contract (capabilities, draft schema, terminal events, normalize/validate) as a `Platform` subclass extending `egress/platforms/base.py`; `registry.py` became a thin delegating facade. Adding a platform is now one file instead of edits across many registry tables. SMTP is a data-only adapter for the SMTP backend. |
| 82 | **On-prem repos via host field** | A per-repo `host` field distinguishes cloud (empty or `bitbucket.org`/`github.com`) from self-hosted (Bitbucket Server / Data Center, GitHub Enterprise). Missing host defaults to Cloud. Threads through detection, ingestion filters, and egress routing. |
| 83 | **Lenient max_tokens with auto-clamp** | `DEFAULT_MAX_TOKENS` raised to 65536 (a ceiling, not a target) to stop JSON-schema truncation on verbose local models, then auto-clamped down to each model's advertised output ceiling so the high default never trips a 400 on strict providers (vLLM/OpenAI/Anthropic). |
| 84 | **Processing-rule firing log** | Every processing-rule firing is recorded in `processing_rule_firings` with its actions, per-action results, and error; queryable/searchable by outcome (success/error/skipped) to make rule effectiveness and failures visible. |
| 85 | **Card created_at = event time** | `action_cards.created_at` (and group active time) hold the source event's platform timestamp rather than insert time, so late-ingested cards show their real time. True ingest time remains on `events.created_at`. Forward-only; no backfill. |
