...
**Agents** (`engine/laya/agents/`): CLI coding agent adapters (Claude Code, Gemini CLI, Codex CLI, Pi CLI, Cursor Agent CLI) for the workspace feature. Abstract protocol in `base.py`, session management in `session_manager.py`. Cursor (`cursor_cli.py`, binary `agent`) has modes `plan` (`--mode=plan`, read-only) / `acceptEdits` (default: edits auto-applied, shell rejected) / `force` (`--force`); research sessions collapse plan → default so the research doc can be written. It never passes `--model` (Cursor persists that as the user's default in `~/.cursor/cli-config.json`), MCP wiring is deferred (no `--mcp-config` flag), and because `agent` is a generic name `config.detect_agent_paths()` validates the binary (realpath contains `cursor-agent`, else `--version` build id) instead of trusting the first PATH hit.
